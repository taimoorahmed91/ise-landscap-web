    ...
    $result = $mysqli->query($query) or die($mysqli->error.__LINE__);
        if($result = $mysqli->query($query)){
                ...
                while($row = $result->fetch_assoc()) {
                        ...
                        $authenticationname = $row['authenticationname'];
                }
                //Free Result set
                $result->close();
        }
...
system("sudo -S python3 /root/ise-landscape/policyset/insert_dstpolicyid.py  $id  $dstise $dstpolicysetid '$authenticationname'");
